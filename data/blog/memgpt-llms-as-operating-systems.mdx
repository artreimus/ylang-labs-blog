...
```mermaid
graph TD
    A[New Messages] --> B[Active Context Window]
    B --> C[Summarization]
    C --> D[Archival Storage]
    D --> E[Retrieval When Needed]
```
...
```mermaid
graph LR
    A[Memory Sections] --> B[Core Memory]
    A --> C[Recall Memory]
    A --> D[Archival Memory]
    B --> E[User Data]
    B --> F[Agent State]
...
```mermaid
graph TD
    A[User Input] --> B[Memory Analysis]
    B --> C{Requires Update?}
    C -->|Yes| D[Edit Memory]
    C -->|No| E[Continue]
    D --> F[Update Core Memory]
    D --> G[Update External Storage]
    F --> H[Next Interaction]
    G --> H
    E --> H
```
...
```mermaid
graph LR
    A[Input Processing] --> B[Inner Monologue]
    B --> C[Memory Check]
    B --> D[Tool Usage]
    B --> E[Response Planning]
    C --> F[Final Response]
    D --> F
    E --> F
```
...
```mermaid
graph TD
    A[Heartbeat Trigger] --> B[State Evaluation]
    B --> C[Memory Check]
    C --> D{Action Needed?}
    D -->|Yes| E[Execute Action]
    D -->|No| F[Wait for Next Heartbeat]
    E --> G[Update State]
    G --> F
    F --> A
```
...
```mermaid
graph TD
    A[Agent State] --> B[System Instructions]
    A --> C[Core Memory]
    A --> D[Recent Messages]
    B --> E[Context Window]
    C --> E
...
```mermaid
graph LR
    A[User Query] --> B[Memory Analysis]
    B --> C[Core Memory Search]
    B --> D[Recall Memory Search]
    B --> E[Archival Search]
...
```mermaid
graph TD
    A[Original Content] --> B[Initial Summary]
...
```mermaid
graph TD
    A[Active Session] --> B[State Capture]
