...
4. **An output projection, often called the language-model head, turns the final hidden state into logits**, one score for each vocabulary token.
...
<Image
  alt="A conceptual next-token prediction pipeline showing prompt tokenization, vector embeddings, fixed Transformer layers, logits, one selected token, and the selected token appended to the context for the next pass"
  src="/static/images/blogs/vllm-inference-engine-explained/prompt-to-next-token.png"
  width={1774}
  height={887}
...
<DiagramSubtitle>
  A decoder-only Transformer converts the current context into logits, selects one next token, and
  appends that token before repeating the forward pass. Model weights stay fixed during inference.
  Conceptual illustration; exact layer and sampling details vary by model and configuration.
</DiagramSubtitle>
...
<Image
  alt="A conceptual inference workload showing prompt tokens processed together through several parallel prefill lanes, followed by a first-token boundary and a sequential autoregressive decode chain"
  src="/static/images/blogs/vllm-inference-engine-explained/prefill-decode-workload.png"
  width={1536}
  height={1024}
  className="mx-auto rounded-md bg-white"
/>
...
<DiagramSubtitle>
  Prefill can process many prompt positions together. Decode remains autoregressive: each accepted
  output token becomes part of the context for the next step. Conceptual illustration; not a
  performance trace.
</DiagramSubtitle>

<MermaidDiagram
  label="Prefill and decode latency timeline"
  description="TTFT ends at the first output token. ITL measures gaps between successive streamed outputs, while TPOT averages post-first-token generation time per output token for each request."
>
  {`
    flowchart LR
...
<DiagramSubtitle>
  A service can have a good TTFT and poor streaming speed, or the reverse. Measure TTFT separately
  from ITL and TPOT.
</DiagramSubtitle>

The core serving metrics describe different user experiences. vLLM's [benchmark CLI](https://docs.vllm.ai/en/latest/benchmarking/cli/) reports ITL and TPOT separately:
...
<Image
  alt="A conceptual KV-cache diagram showing prompt prefill writing paired key and value state for earlier tokens, while decode reads the stored history and appends one complete new key-value pair"
  src="/static/images/blogs/vllm-inference-engine-explained/kv-cache-function.png"
  width={1774}
  height={887}
  className="mx-auto rounded-md bg-white"
/>
...
<DiagramSubtitle>
  Prefill creates key and value state for the prompt. Each ordinary decode step reads the existing
  history, computes the new token state, and appends one new pair. The geometry is conceptual; real
  cache layouts depend on the model and serving configuration.
</DiagramSubtitle>
...
<Image
  alt="A PagedAttention mapping plate showing four ordered logical KV-cache blocks routed through a block table to four scattered fixed-size blocks in a shared physical memory pool"
  src="/static/images/blogs/vllm-inference-engine-explained/pagedattention-block-mapping.png"
  width={1536}
  height={1024}
  className="mx-auto rounded-md bg-white"
/>
...
<DiagramSubtitle>
  Each table entry maps one ordered logical block to a physical block. Yellow blocks belong to the
  illustrated request, green blocks belong to other requests, and unfilled blocks remain available.
  Conceptual illustration; not a literal memory dump.
</DiagramSubtitle>
...
<Image
  alt="A token-budget scheduler plate showing three consecutive engine iterations that prioritize active decode slots, use remaining capacity for bounded prefill chunks, and admit or retire requests between iterations"
  src="/static/images/blogs/vllm-inference-engine-explained/continuous-batching-scheduler.png"
  width={1774}
  height={887}
  className="mx-auto rounded-md bg-white"
/>
...
<DiagramSubtitle>
  Each green cell represents one ordinary decode token for one active request. The yellow cells
  collectively spend the remaining iteration budget on a bounded prefill chunk. Requests can join or
  leave between iterations. Conceptual illustration; not fixed token counts or benchmark data.
</DiagramSubtitle>
...
<Image
  alt="A first request populating completed shared-prefix KV blocks, followed by an exact-match request reusing those cached blocks while computing its unique tail and new decode tokens"
  src="/static/images/blogs/vllm-inference-engine-explained/prefix-cache-reuse.png"
  width={1536}
  height={1024}
  className="mx-auto rounded-md bg-white"
/>
...
<DiagramSubtitle>
  A later request can reuse completed KV blocks only when its token prefix matches exactly. Its
  unique tail and every new output token still require model execution. Conceptual illustration;
  eviction and cache-key details are omitted.
</DiagramSubtitle>
...
<Image
  alt="A vLLM V1 process architecture plate showing clients communicating with an API server, a separate engine core coordinating scheduling and KV state, and multiple GPU worker processes executing the model"
  src="/static/images/blogs/vllm-inference-engine-explained/vllm-v1-architecture.png"
  width={1536}
  height={1024}
  className="mx-auto rounded-md bg-white"
/>
...
<DiagramSubtitle>
  The API server handles request-facing work, the engine core schedules tokens and coordinates KV
  state, and GPU workers execute model operations. Data-parallel deployments add engine cores and
...
11. vLLM Project, [Benchmark CLI](https://docs.vllm.ai/en/latest/benchmarking/cli/).
