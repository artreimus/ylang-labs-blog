...
tldr: |
  ...
  - Agents need selected context and bounded sandboxes, not unlimited repo access and vague goals.
...
Autonomous software development is not an agent opening a pull request after one prompt. That is a demo.

The real step forward is the system around the agent: clear intake, useful context, sandboxed work, verification, review, and memory.

Autonomy without control is just a faster mess. If an agent turns vague tickets into unchecked patches, you did not build a factory. You built code generation with permissions.
...
Run the work in a sandbox: a disposable worktree, container, or test environment where the agent cannot damage shared state.

That sandbox still needs limits: selected context, allowed files, tool permissions, validation commands, and a stop condition. Autonomy works better when the room has walls.
