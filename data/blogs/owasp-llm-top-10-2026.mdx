...
The official 2026 PDF describes a corpus of 7,714 public incidents, with 6,639 containing enough detail to classify. OWASP says the community vote carries three-quarters of the final weight and incident evidence carries one-quarter. The project says Prompt Injection remains first despite falling outside the top ten when categories are ranked by raw incident count, while Misinformation moved upward because incident evidence placed it higher than practitioners did. Those are OWASP’s methodology claims, not an independent prevalence study. Treat the list as a prioritization signal, then measure your own exposure.
...
Jailbreaking is a subset of direct Prompt Injection aimed at bypassing the model’s safety behavior. Other variants split or disguise instructions across fields, languages, Base64, invisible Unicode, images, audio, or video. The delivery format changes the defense surface; it does not turn untrusted content into trusted instructions.
...
The model may choose tools dynamically and make repeated calls using earlier outputs as context. Unexpected behavior can also originate in a compromised tool or peer agent, not only in the model’s own hallucination or a user’s prompt.
...
<Image
  alt="An original LLM supply-chain threat model showing data and model sources flowing through training, model repositories, conversion, development, inference, application, and optional on-device deployment, with red threat tags and a threat legend"
  src="/static/images/blogs/owasp-llm-top-10-2026/supply-chain-threat-model.png"
  width={1672}
  height={941}
...
<DiagramSubtitle>
  Original editorial reinterpretation of OWASP Figure 3. The diagram preserves the source’s
  lifecycle and attack-surface idea while using new artwork, labels, and composition; it is not an
  official OWASP reproduction.
</DiagramSubtitle>
...
For systems with MCP servers, tool registries, or other agent-specific components, OWASP places the agentic supply-chain treatment in its separate Agentic Applications list. LLM04 still covers the model, package, and deployment supply chain around the model-as-component system.
...
Severity depends on what the hidden context contains and how much the application relies on its secrecy: disclosure may be informational when it reveals harmless implementation detail, or become high/critical when it exposes credentials or chains into privilege escalation, broad exfiltration, or remote code execution.
...
Depending on the sink and the application’s privileges, exploitation can lead to XSS or CSRF in a browser, SSRF, privilege escalation, or remote code execution on a backend. Markdown images, link previews, iframes, terminals, logs, and IDEs also deserve attention because they may render output or fetch external resources automatically.
...
2. [OWASP Top 10 for LLM Applications 2026 PDF](https://genai.owasp.org/download/56857/?tmstv=1785822482), OWASP Gen AI Security Project, accessed August 23, 2026.
