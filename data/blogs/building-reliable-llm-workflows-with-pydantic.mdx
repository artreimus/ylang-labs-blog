...
draft: false
summary: 'Make Pydantic your LLM contract: schema-first prompts, strict validation (including tool args), structured retries and logging, provider-agnostic wrappers, and versioned schemas—so bad JSON and rogue enums never reach production.'
...
- Python `3.10+`
- Pydantic `>= 2.0`
- Access to at least one LLM provider (e.g., OpenAI, Anthropic, Gemini) capable of JSON‑like outputs
...
- Pydantic for LLM Workflows (DeepLearning.AI): https://learn.deeplearning.ai/courses/pydantic-for-llm-workflows/lesson/w6ohb/welcome-to-pydantic-for-llm-workflows
