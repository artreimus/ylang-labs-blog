...
The Hacker News thread behind this brief is useful as a signal, not as proof by itself. Builders are noticing that agent performance often gets worse because the tool layer is noisy.
...
<MermaidDiagram>
  {`
    flowchart LR
      A["User task"] --> B["Large tool catalog loaded"]
      B --> C["Ambiguous tool choice"]
      C --> D["Raw result payload"]
      D --> E["Missing next-step ID"]
      E --> F["Repair call"]

      A2["User task"] --> B2["Search or load only relevant tools"]
      B2 --> C2["Narrow tool choice"]
      C2 --> D2["Shaped result"]
      D2 --> E2["Next-step ID included"]
      E2 --> F2["Continue"]
...
There is an uncomfortable tradeoff in tool descriptions.
...
Measure the full loop: request tokens, response tokens, latency, number of tool calls, repair calls, and final task success.
