...
**Agentic RAG** is active. Instead of relying on static context, the agent actively formulates its own search queries based on the _current step's goal_. It decides _what_ to search for, _when_ to search, and _how_ to refine the query if the first result is noise.

A common mistake in the retrieval layer itself is "embedding-only retrieval." Semantic similarity is necessary, but it is not sufficient. In practice, retrieval works best when you blend at least three signals:
...
Common placement choices include **System instructions** (authority), **Conversation history** (easy access), **Memory-as-a-tool** (on-demand), or **Skill instructions** (task-specific).
