...
description: Create beautiful, repeatable image-generation prompts and raster section visuals for Ylang Labs technical blog posts. Use this skill when the user asks for inline section images, technical diagrams, exploded views, blueprint diagrams, architecture plates, process maps, system diagrams, product-teardown visuals, spec sheets, or OpenAI image-generation prompts in the clean editorial technical style of the provided reference images. For normal Ylang Labs blog cover artwork, `cardImage.png`, and `blogHeader.png`, use `oil-painting-image-generator` first, then `blog-image-cropper`; use this skill for cover/card/header assets only when the user explicitly asks for a technical-diagram cover. Always pair generation with OpenAI's `imagegen` skill and the latest official GPT Image / ChatGPT image generation path; do not create SVG/vector/code-native diagram sources as part of this skill unless the user explicitly asks for manual vector work.
---

# Technical Blog Image Generator

Use this skill to create technical diagram imagery for Ylang Labs posts using OpenAI image generation: clean, precise, editorial, and repeatable. The output should feel like a technical manual, blueprint plate, museum catalog, and engineering blog figure had the same design system.
...
- Maintain legibility in the target placement. Dense section visuals, wide placements, and portrait variants usually need separate compositions, not a blind crop.
...
Use for dramatic technical plates, productized modules, AI-agent systems, interface cards, and launch visuals.
...
Canvas: [aspect ratio and target use, e.g. square inline plate, wide section figure, or explicitly requested technical cover/card crop].
...
State any limitation clearly, especially if image generation produced imperfect text or if a source image is too dense for the requested placement.
